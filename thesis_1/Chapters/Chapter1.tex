...
\textcolor{red}{Traditional machine learning methods of anomaly detections in
	sensor data are based on domain-specific feature engineering. A typical approach is to
	use domain knowledge to analyze sensor data and manually create statistics-based
	features, which are then used to train the machine learning models to detect and classify
	the anomalies. Although this methodology is used in practice, it has a significant
	drawback due to the fact that feature extraction is usually labor intensive and requires
	considerable effort from domain experts. }
...
\begin{figure}[h]
	...
	\decoRule
	\caption[Kernel]{Example of a Filter used in CNN \parencite{RichStureborg2019}}
	\label{fig:filter}
\end{figure}
...
\begin{figure}[h]
	...
	\decoRule
	\caption[Pooling]{Example of max-pooling \parencite{RichStureborg2019}}
	\label{fig:pooling}
\end{figure}
